...
if not event or cjson.decode(event).ends_at <= now then
  return 'GONE'
end
...
__CLOCK__
local raw = redis.call('GET', KEYS[1])
local challenge = redis.call('GET', KEYS[2])
local event = redis.call('GET', KEYS[3])
if not raw or not challenge or not event then
  return 'GONE'
end
local s = cjson.decode(raw)
local nonce = cjson.decode(challenge)
local g = cjson.decode(event)
if
  s.expires_at <= now
  or g.ends_at <= now
  or nonce.expires_at <= now
  or nonce.hash ~= ARGV[1]
  or nonce.gathering ~= g.id
  or s._gathering ~= g.id
  or g.intersection.id ~= ARGV[3]
then
  return 'GONE'
end
...
if redis.call('PTTL', KEYS[4]) < g.ends_at - now then
  redis.call('PEXPIRE', KEYS[4], g.ends_at - now)
end
...
for _, member in ipairs(expired) do
  ...
  if split then
    local key = 'gati:s:' .. string.sub(member, 1, split - 1)
    local value = redis.call('GET', key)
    if value then
      local s = cjson.decode(value)
      if s._arrival_member == member then
        s._arrival_member = nil
        s.arrival_until = nil
        if s.state == 'here' then
          s.state = 'going'
...
local cohort = redis.call('GET', KEYS[3])
local valid = false
local pending = nil
if cohort then
  pending = cjson.decode(cohort)
  valid = true
...
elseif not valid then
  ...
  redis.call(
    ...
    value,
    'PX',
    math.min(g.ends_at - now, tonumber(ARGV[2]) + tonumber(ARGV[4]))
  )
end
