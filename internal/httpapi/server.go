...
func Handler(c config.Config, backend *store.Store, roads []byte, engines ...*worker.Engine) http.Handler {
	...
	mux.HandleFunc("POST /api/gathering-preview", signal.preview)
	mux.HandleFunc("POST /api/arrival-nonce", signal.arrival)
	mux.HandleFunc("POST /api/arrival", signal.arrival)
	mux.HandleFunc("POST /api/arrival-renewal-nonce", signal.arrival)
	mux.HandleFunc("POST /api/arrival-renewal", signal.arrival)
	mux.HandleFunc("DELETE /api/arrival", signal.arrival)
