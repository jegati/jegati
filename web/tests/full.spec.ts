...
test('real publisher journey through fresh arrival, public map, preview and late admission', async ({ page, context, request, browser }) => {
  ...
  expect(config.public_activity.release_seconds).toBe(300);
  ...
  try {
    ...
    await test.step('confirm independent synthetic credentials through real arrival API',async()=>{
      ...
      await context.setGeolocation({longitude:destination[0],latitude:destination[1],accuracy:20});
    ...
    await test.step('wait for the normal delayed real public release',async()=>{
      await expect(page.locator('#here-count')).toContainText('20+', {timeout:960000});
      ...
      expect(release.release_at-release.observed_from).toBeGreaterThanOrEqual(300000);
