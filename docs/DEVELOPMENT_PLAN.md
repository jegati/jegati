...
The first screen offers availability duration, travel radius, required one-time device-location permission (no manual location input), and **JAM GATI**. No account, onboarding sequence, profile, feed, chat, organizer role, user-generated event descriptions, attendance pressure, or participation history. Display “Gatishmëria nuk është premtim për të marrë pjesë.” Cancellation is immediate and neutral. Use 🦩 as the simple brand symbol; all user-facing copy, accessibility labels, errors, notifications, and installation instructions are Albanian.

The initial activity is gathering near an automatically selected mapped road intersection (crossroad). There is no manually predefined meeting-area catalog. Continuous matching finds a compatible group and selects the nearest eligible crossroad to its coarse group center, subject to every counted participant's travel radius. The destination refers to pedestrian space beside the intersection, not standing in the roadway. A notification names the intersection and gathering deadline. Newly notified people can join an active gathering and confirm arrival without having participated in its original activation. Availability starts at 30 minutes; gathering formation is not tied to scheduled time slots.
...
2. The client requires a fresh device-service fix, with configurable age and reported-accuracy checks and no manual fallback. These are client quality checks, not location attestation. Exact coordinates are processed on the device, converted to a coarse cell, and never sent to GATI. The server necessarily sees coarse cells and temporary session state. Request timing, IP addresses at the network boundary, and optional push endpoints remain privacy risks.
...
4. Decision 0004 retains deterministic nearest-crossroad selection with the demonstrated colluding-input limitation; the absolute inference claim remains unmet. Fixed, delayed, bucketed releases reduce public inference. They do not mathematically guarantee anonymity against collusion, observation at a meeting, or an adversary controlling most inputs. Thresholding alone is not differential privacy.
...
| Durable content | Reviewed files and static artifacts | Imported intersection dataset, schemas, public configuration and protected aggregate summaries need no SQL service initially |
...
The API accepts a documented coarse-cell format, never participant coordinates. Start with a fixed geographic grid and a spatial index of road intersections imported from a versioned OpenStreetMap extract for Tirana. Precompute static cell/radius-to-intersection reachability; choose destinations dynamically from that map data. Use a reviewed geographic library for distance calculations and test cell boundaries. Publish the grid, import recipe, source checksum/date, attribution, filtering rules and resulting intersection dataset. Intersection coordinates are public map features, not participant locations. Basemap requests come from the same origin and do not use an external geocoder, font CDN, or map SDK telemetry.
...
### Continuous matching and automatic crossroad selection
...
- For each proposed group, calculate the mean of its members' coarse-cell centers. Define **closest crossroad** as the eligible mapped intersection with the shortest geographic distance to that coarse group center, restricted to intersections reachable by every counted member. Break ties by stable map-feature ID. Never use exact device locations for selection or publish the computed center. A selected intersection still reveals information about the coarse group location: include nearest-intersection selection and repeated candidate probes in the inference review. Revalidate membership, destination and deadlines before activation.
- Derive intersections from shared OSM street nodes with at least three distinct adjacent street nodes [8]. Normalize by node ID; split ways, bends and geometry-only overlaps are not crossroads. Include public street classes; omit explicit access prohibitions, construction/disused/private, motorway/trunk, bridge/tunnel and motorroad features. Service driveways, ramps and paths do not contribute junction legs. Roundabout entry nodes may qualify; do not invent a central-island destination. Retain public source IDs and document omitted metadata/coverage in data/tirana/README.md. The point is a landmark beside which users choose pedestrian space; map data cannot certify space or crowd capacity.
- If no eligible shared intersection is mapped, keep willingness active and display an Albanian explanation that no suitable mapped meeting point was found. Do not invent a destination or silently fall back to a predefined plaza. Intersections may be excluded through published map corrections, but normal formation must not depend on an operator approving each location.
- Map imports are offline administrative jobs, never per-participant external queries. Pin an import version during matching; updates apply to new gatherings. Once activated, the intersection and deadline stay fixed, including when late participants join. If a intersection is later marked unavailable, close the gathering with a neutral update rather than silently moving people.
...
- Fixed public areas, one published geographic resolution, and globally aligned five-minute releases with a one-epoch delay. No user-selected aggregate query radius, time range, availability filter, or exact count endpoint. The chosen travel radius filters eligibility privately; it does not produce a custom public count. Public gathering cards identify only the containing public cell, using the same grid as willingness; they contain no intersection ID, label or coordinates (decision 0007). Private invitations/admission retain actionable crossroads. Include a snapshot timestamp and live admission revalidation when opening its join action.
...
- Decision 0007 accepts inferential leakage for implementation: document collusion/differencing failures without disabling the agreed numeric surfaces. Keep fixed cells, suppression, buckets and delay; direct individual/raw-count disclosure is still forbidden. Formal differential privacy with an explicit contribution model and privacy budget is a later specialist option, not a home-grown noise function.
...
| Map extract, intersection index, schema, build/config artifacts | Everyone | Durable, contains no participant activity |
...
For 100,000 live signals, precomputed compatibility and incremental cell/time/radius counters avoid all-pairs matching. Re-evaluate affected intersection candidates with bounded batches and stability/expiry timers, then atomically validate before activation. Set explicit limits on intersection-index batch size per cell, membership allocation, worker batch size, notification fanout and outstanding requests. Backpressure must preserve expiry and prevent unbounded queues.
...
| --- | --- | --- |
| 01 | `docs: define GATI product and threat boundaries` | Preserve requirements, this plan, continuous matching/late-join/intersection ADRs, data inventory and claim/evidence matrix. Record unresolved guarantees and selected MVP semantics. |
...
| 03 | `feat: define coarse geography and Tirana fixtures` | Versioned grid, reproducible OSM road-junction import/index, conservative reachability table and local attributed basemap. Test intersection normalization, closest eligible selection, missing map data and boundaries; no handpicked meeting catalog. |
...
| 05 | `feat: build Albanian willingness flow` | Minimal responsive accessible screen, availability ≥30 minutes/radius, required fresh device location, coarsened locally, JAM GATI and cancellation. First useful action in seconds; inspect network/storage for location or persistent IDs. |
...
| 07 | `feat: activate compatible gatherings` | Continuous matching/timers, nearest eligible mapped intersection, single allocation, stable configurable threshold and fenced/idempotent worker. JEMI GATI, late admission, going and decline-without-withdrawal. Test arbitrary-minute availability, ties, missing intersections, destination stability, concurrent joins, expiry and worker retries. |
...
| 09 | `feat: publish protected collective map` | Fixed delayed snapshots, suppression/buckets, no markers, safe history summary and shared public cache. Tests for complementary totals, temporal differencing, private activation probes and cache leakage; document accepted inference and reject direct record/coordinate disclosure. |
...
| 12 | `perf: validate 100k signals and traffic bursts` | Real-time load harness and reproducible reports; optimize only measured bottlenecks. Include dense intersection datasets, continuous update storms and mass late admission; meet agreed reference-host targets or revise deployment sizing before claiming scale. |
...
| 2. Private geographic matching | On-device coarsening, fixed grid, nearest shared reachable mapped intersection | Coordinate-rejection and cell-boundary tests; live store still contains coarse area |
...
| 4. JEMI GATI | Continuous configurable threshold for a shared mapped intersection and overlapping remaining availability; late joining; one allocation per signal | Deterministic, race and adversarial enrollment scenarios |
