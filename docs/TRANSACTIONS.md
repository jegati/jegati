...
| [arrivals.go](../internal/store/arrivals.go): `issueNonce`, `confirmArrival`, `retractArrival`, `reconcilePresence` | One-use challenges, current intent/claim, arrival expiry, bounded presence index, stability and threshold transitions | [concurrent replay and seeded transition model](../internal/store/arrivals_test.go) |
