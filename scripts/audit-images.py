...
def classify(service, findings, exceptions, today):
    ...
    for item in findings:
        matches = [
            e
            for e in exceptions
            if service in e["services"]
            and e["id"] == item["id"]
            and e["package"] == item["package"]
            and e["version"] == item["version"]
        ]
        valid = [
            e
            for e in matches
            if datetime.date.fromisoformat(e["reviewed"])
            <= today
        ...
        if len(valid) == 1:
            accepted.append({**item, "exception": valid[0]})
        else:
...
def audit(release, out, trivy):
    ...
    manifest = verify_files(release)
    verify_images(manifest)
    expected_version = dict(
        line.split("=", 1)
        for line in (ROOT / "scripts/toolchain/versions.env").read_text().splitlines()
        if line and not line.startswith("#")
    )["GATI_TRIVY_VERSION"]
    version = json.loads(
        subprocess.check_output([trivy, "version", "--format", "json"])
    )
    if version["Version"] != expected_version:
        raise ValueError("unexpected scanner version")
    exceptions = json.loads((release / "deploy/security-exceptions.json").read_text())[
        "exceptions"
    ]
    ...
    result = {
        ...
        "scanner": version,
        "scope": "local image inventory plus Go binary symbol checks; no exploitability or remote attestation guarantee",
        "images": {},
    }
    with (
        tempfile.TemporaryDirectory(prefix="gati-image-audit-") as temporary,
        (out / "commands.log").open("wb") as log,
    ):

        def run(*args):
            return subprocess.check_output(args, stderr=log).decode().strip()
        ...
        for name, image in manifest["images"].items():
            ...
            if not scan.get("Results"):
                raise ValueError("scanner produced no package results")
            ...
            if name in BINARIES:
                ...
                container = run("docker", "create", "--network", "none", identity)
                try:
                    run("docker", "cp", container + ":" + BINARIES[name], str(binary))
                finally:
                    run("docker", "rm", container)
                entry["binary_sha256"] = hashlib.sha256(binary.read_bytes()).hexdigest()
                if entry["binary_sha256"] != manifest["runtime_binary_sha256"][name]:
                    raise ValueError("scanned binary differs from recorded release")
            ...
            passed = passed and not unresolved
            print(
                name
                + ": "
                + str(len(unresolved))
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        default=str(
            pathlib.Path(
                os.environ.get(
                    "GATI_TOOLS_DIR",
                    str(pathlib.Path.home() / ".local/share/gati/tools"),
                )
            )
            / "trivy-0.74.0/trivy"
        ),
    ...
    a = p.parse_args()
    raise SystemExit(
        0 if audit(a.release.resolve(), a.output.resolve(), a.trivy) else 1
    )
