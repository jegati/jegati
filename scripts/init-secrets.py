...
acl=['user default off',
     ...
     'user app on #'+hashlib.sha256(passwords['app'].encode()).hexdigest()+' ~'+prefix+'* +ping +hello +get +set +exists +del +pttl +pexpire +pexpireat +zadd +zrem +zrangebyscore +zcard +eval +evalsha +script|load +time +incr']
p=root/'users.acl'
content='\n'.join(acl)+'\n'
