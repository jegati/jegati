...
class ReleaseTests(unittest.TestCase):
    def test_existing_acl_changes_are_rejected_without_rewrite(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            acl = pathlib.Path(d) / "users.acl"
            acl.chmod(0o600)
            acl.write_text("synthetic changed ACL")
            self.assertNotEqual(
                subprocess.run(
                    args + ["--check"],
    ...
    def test_releases_share_operational_mounts(self):
        self.assertEqual(
            runtime_dir(pathlib.Path("/opt/gati/releases/a"), "gati-production"),
            runtime_dir(pathlib.Path("/opt/gati/releases/b"), "gati-production"),
        )
...
class RuntimeTests(unittest.TestCase):
    def setUp(self):
        ...
        self.service = {
            "command": ["-trusted-proxies", "172.30.11.2/32"],
            ...
            "volumes": [
                {
                    "target": "/config/gati.yaml",
                    "type": "bind",
                    "source": "/release/config/gati.yaml",
                    "read_only": True,
                }
            ],
            "tmpfs": ["/run/ops:mode=0700,size=1m"],
            "networks": {"application": {"ipv4_address": "172.30.11.2"}},
        }
        ...
        self.value = {
            ...
            "NetworkSettings": {
                "Networks": {"test_application": {"IPAddress": "172.30.11.2"}}
            },
        }

    def check(self, value):
        verify_runtime_shape(value, self.service, self.networks, self.image)

    def test_expected_launch_passes_with_image_defaults(self):
        self.check(self.value)

    def test_launch_drift_rejected_without_disclosing_values(self):
        for field, replacement in [
            ("Cmd", ["-trusted-proxies", "0.0.0.0/0"]),
            ("Entrypoint", ["/alternate"]),
            ("User", "0"),
            ("Env", ["PRIVATE_TEST_SECRET=must-not-print"]),
            ("WorkingDir", "/alternate"),
        ]:
    ...
    def test_network_or_proxy_address_drift_rejected(self):
        for networks in [
            {},
            {"test_application": {"IPAddress": "172.30.11.3"}},
            {**self.value["NetworkSettings"]["Networks"], "egress": {}},
        ]:
            value = copy.deepcopy(self.value)
    ...
    def test_omitted_live_tunnel_or_worker_rejected(self):
        for target in ["worker", "tunnel"]:
            with patch(
                "release.compose",
                side_effect=lambda *args, **kw: "container"
                if args[-1] == target
                else "",
            ):
                with self.assertRaises(ValueError):
                    verify_service_selection(pathlib.Path("/release"), "test", 1, False)

    def test_selected_services_are_not_treated_as_orphans(self):
        with patch("release.compose") as command:
            verify_service_selection(pathlib.Path("/release"), "test", 2, True)
            command.assert_not_called()
...
class AuditGateTests(unittest.TestCase):
    def test_public_activation_requires_exact_fresh_complete_audit(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            manifest = {
                "images": {"api": {"image_id": "sha256:image"}},
                "runtime_binary_sha256": {"api": "binary"},
            }
            ...
            verify_audit(root, manifest, path, now)
            with self.assertRaises(ValueError):
                verify_audit(root, manifest, None, now)
            for change in [
                {"passed": False},
                {"scanned_at": (now - datetime.timedelta(days=2)).isoformat()},
                {"scanned_at": (now + datetime.timedelta(hours=1)).isoformat()},
                {"release_manifest_sha256": "other"},
                {"images": {}},
            ]:
            ...
            for change in [
                {"image_id": "other"},
                {"binary_sha256": "other"},
                {"binary_scan_passed": False},
                {"unresolved_findings": ["new"]},
                {"accepted_findings": [{"exception": {"expires": "2026-09-13"}}]},
            ]:
                changed = copy.deepcopy(audit)
                changed["images"]["api"].update(change)
