...
def rehearse(source,out):
 ...
 verify_files(source);project='gati-release-lab-'+os.urandom(4).hex();connector=project+'-connector';started=time.time()
 with tempfile.TemporaryDirectory(prefix='gati-release-rehearsal-') as directory,(out/'commands.log').open('wb') as log:
  parent=pathlib.Path(directory);a=parent/'a';b=parent/'b'
  shutil.copytree(source,a);shutil.copytree(source,b,copy_function=os.link)
  ...
  try:
   ...
   port=int(command('docker','port',connector,'8090').rsplit(':',1)[1]);base=f'http://localhost:{port}'
   ...
   def request(method,path,data=None):
    ...
    try:
     c.request(method,path,json.dumps(data) if data is not None else None,{'Authorization':'Bearer '+cap,'Content-Type':'application/json'});r=c.getresponse();raw=r.read(50000);return r.status,json.loads(raw) if raw else None
    finally:c.close()
   status,created=request('POST','/api/signals',{'cell':'tirana-v1:100:55:55','radius_km':3,'availability_minutes':30});assert status==200
   ...
   assert json.loads(command('docker','inspect',store))[0]['State']['StartedAt']==before
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--release',type=pathlib.Path,required=True);p.add_argument('--output',type=pathlib.Path,required=True);a=p.parse_args();rehearse(a.release,a.output)
